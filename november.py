# -*- coding: utf-8 -*-
"""
Created on Sun Dec 18 11:39:42 2016
 ...
 #   ax.set_ylim(0,40)
  #  plt.legend()
    ...
    if plot_hours:
        ...
        datemin = datetime.date(2016, 11, 1)
        datemax = datetime.date(2016, 12, 1)
    ...
    for i in range(0,len(thermo_dataset)):
        ...
        if(len(temp_series.tolist()) > 0):
            ...
            if not_nan_pct > 0.25:
                ...
                dates = pd.date_range('2016-10-31 23:30:00', '2016-12-01 05:00:00', freq = '10Min')
...
def remove_outliers(thermo_data, m):
    ...
    for i in range(0, len(thermo_data)):
        ...
        if  corr > 0.3 and var_station > 2 :  #some var needed to prevent runtime errors..
            print(corr)
            temps = []
            temps = temps+temp_series['20161031'].tolist()
            
            for i in range(1,31):
                date = datetime.datetime(2016,11,i)
            ...
            temps = temps + temp_series['20161201'].tolist()
            z = []
...
def pca(data, name):
    ...
    new_data = [{'latitude':d['latitude'], 'longitude':d['longitude'], 'name':d['name'], 'measurements': d['measurements']['2016-10-31']} for d in data]
    for j in range(1,31): 
...
def kalman(station, model, m, i):
    ...
    for k in range(1,n_iter):
      ...
      #  K[k] = Pminus[k]/( Pminus[k]+R ) #static R
        ...
        if np.isnan(z[k]):
            ...
            if abs(np.round(Pminus[k],1)+np.round(R[k],1)) < 0.5:
                R[k] = R[k]+ 1
            K[k] = Pminus[k]/(Pminus[k]+R[k]) #dynamic R
            xhat[k] = xhatminus[k]+K[k]*(z[k]-xhatminus[k])
            
            P[k] = (1-K[k])*Pminus[k]
            if xhat[k] > 20 or xhat[k] < -10:
                print('xhat: {}, P: {}, K: {}, Pm: {}, xhatm: {}, R: {}'.format(xhat[k], P[k], K[k], Pminus[k], xhatminus[k], R[k]))
    ...
    if PLOT:
        ...
        ax.xaxis.set_major_locator(days)
        ax.xaxis.set_major_formatter(daysFMT)
        datemin = datetime.date(2016, 11, 9)
        datemax = datetime.date(2016, 11, 15)
        ax.set_xlim(datemin, datemax)
        plt.legend(loc=4)
...
def create_knmi():
    ...
    dates = pd.date_range('2016-11-01 00:00:00', periods = len(df.value), freq='10Min')
    d = pd.DatetimeIndex(dates)
...
def plot_data(thermo_data, name):
    import matplotlib.pyplot as plt
    fig,ax = plt.subplots()
    data = [d['measurements']['2016-11'].tolist() for d in thermo_data]
    import numpy as np
    ...
    ax.plot(knmi_timeseries['2016-11'].tolist(), array_data.T, '.', ms=1.4, color='seagreen')
    min_ax = max(ax.get_xlim()[1], ax.get_ylim()[1])-3
    fig.set_size_inches(5,5)
    ax.plot([-10, min_ax], [-10, min_ax], ls="--", color='k', lw = 2.0, label='perfect correlation' )
    ax.set_xlim(-10,min_ax)
    ax.set_ylim(-10, min_ax)
    cor = np.nanmean(corr_data)
    ax.text(min_ax/1.6, 2, r'$r = {0:.3f}$'.format(abs(cor)), fontsize=15, color='darkred')
    plt.legend(loc =4)
    plt.ylabel('Measurements CWS')
    plt.xlabel('Measurement KNMI')
...
def kriging_fun(sliced, hs, bw, covfct, name, mu, save):
    '''
...
def spatial(data):
    dates = pd.date_range('2016-11-01 00:00:00', '2016-11-30 23:50:00', freq = '10Min')
    ...
    for date in dates:
        ...
        if (date.hour == 12 or date.hour == 6 or date.hour ==18 or date.hour == 0) and date.minute == 0:
            save = True
        mu = mean[dstr]
        Z, mse, err = kriging_fun(sliced, hs, bw, covfct, 'day {} hour {}_00'.format(date.day, date.hour), mu, save)
        mse_time.append(mse)
...
def run_analysis():        
    ...
    mse_series = pd.Series(mse_time, dates)     
    summed = []
    for i in range(1,31):
        date = datetime.datetime(2016,11,i)
        td = datetime.datetime.strftime(date, '%Y%m%d')
        summed.append(mse_series[td].tolist())
    ...
    idx = np.array(corr) > 0.955
    corrbest =  [raw[i] for i in range(len(idx)) if idx[i]==True]
    plot_thermo_mods(corrbest, False, get_mean_faster(raw), 'Best {} corr stations r 0.955 cutoff'.format(len(corrbest)))

    idx = np.array(corr) < 0.1
    corrworst =  [raw[i] for i in range(len(idx)) if idx[i]==True]
    plot_thermo_mods(corrworst, False, get_mean_faster(raw), 'Worst {} corr stations r 0.1 cutoff'.format(len(corrworst)))
    ...
    idx = np.array(mse) < 0.043
    kalmanbest_12 = [outlier[i] for i in range(len(idx)) if idx[i]==True]
    plot_thermo_mods(kalmanbest_12, False, get_mean_faster(outlier), 'Best 10 mse stations mse 0.043 cutoff')
    
    idx = np.array(mse) > 1.4
    kalman_12 = [outlier[i] for i in range(len(idx)) if idx[i]==True]
    plot_thermo_mods(kalman_12, False, get_mean_faster(outlier), 'Worst 10 mse stations mse 1.4 cutoff')
    ...
    plot_thermo_mods(pcworst, False, get_mean_faster(filtered), 'Worst 10 PC var explained stations perc 0.745 cutoff')
    
    idx = var > 0.967
    pcbest = [filtered[i] for i in range(len(idx)) if idx[i]==True]
    plot_thermo_mods(pcbest, False, get_mean_faster(filtered), 'Best 10 PC var explained stations perc 0.967 cutoff')
    ...
    idx = np.array(mse_per_station) >3.3
    krigeworst = [data[i] for i in range(len(idx)) if idx[i]==True]
    plot_thermo_mods(krigeworst, False, get_mean_faster(filtered), 'Worst 10 Kriging mse stations mse 3.3 cutoff')
    
    idx = np.array(mse_per_station) < 0.22
    krigebest = [data[i] for i in range(len(idx)) if idx[i]==True]
    plot_thermo_mods(krigebest, False, get_mean_faster(filtered), 'Best 10 Kriging mse stations mse 0.22 cutoff')
