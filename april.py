 ...
 #   ax.set_ylim(0,40)
  #  plt.legend()
    ...
    if plot_hours:
        ...
        datemin = datetime.date(2016, 4, 14)
        datemax = datetime.date(2016, 4, 22)
 ...
 #   ax.set_ylim(0,40)
  #  plt.legend()
    ...
    if plot_hours:
        ...
        ax.xaxis.set_major_formatter(daysFMT)
        datemin = datetime.date(2016, 4, 14)
        datemax = datetime.date(2016, 4, 21)
        ax.set_xlim(datemin, datemax)
        for label in ax.xaxis.get_ticklabels()[::2]:
            label.set_visible(False)
   # plt.grid() 
...
def remove_outliers(thermo_data, m):
    ...
    return new_data, correlations, z_scores, variation
...
def corr_as_function_of_distance(data, name, resolution, time, Ydist, max_dist):
    ...
    plt.ylabel('Average correlation coeff'.format(time))
...
def fit_gamma(Mn, Ydist, res, max_dist, model='Spherical'):
    hvals = linspace(0, max_dist, np.ceil(max_dist)*res)
    if model == 'Exponential' or model == 'Gaussian' or model == 'Spherical':
   ...
   #      c_null =  (1.2-np.nanmean(M)) *np.nanstd(np.asarray([d['measurements'] for d in data]))
    ...
    for h in hvals:
        ...
        i = i+1
    fig, ax = plt.subplots()
    ax.plot(Mn, '.',label= 'Sample semivariance')
    ax.plot(gamma_h, 'r',lw=2, label = '{} fit'.format(model))
...
def map_temp():
    date = '2016-04-21 15:00:00'
    sliced, temp = plot_points_slice(data, date)
    P = np.array(sliced).T
    idx = np.isnan(P[:,2])
    P = P[~idx]
    ...
    hs = np.arange(0,43000,500)
    covfct = cvmodel( P, gaussian, hs, 500 ) 
    mu = np.nanmean(P[:,2])
    for i in range( 80 ):
        print (i)
        for j in range( 100 ):
            est, err = krige(P, hs, 500, (X0+dx*j, Y0+dy*i), 10, covfct, mu)
            Z.append( (X0+dx*j, Y0+dy*i, est))
    
    fig,ax = plt.subplots()
    plt.scatter([z[0] for z in Z], [z[1] for z in Z], c=[z[2] for z in Z], s=30, marker="s", lw = 0.0, cmap = 'rainbow') 
    ax.scatter([p[0] for p in P], [p[1] for p in P], facecolor = "none", lw=0.75) 
    ax.set_xlim(X0, X1)
    ax.set_ylim(Y0, Y1)
    plt.colorbar(label='temperature in degrees Celsius')
    ...
    plt.savefig('figures\Spatial analysis\Krigingestimate_21_15.png'.format(date), dpi=600)
...
def run_analysis():        
    ...
    outlier, corr, z_scores, variations = remove_outliers(raw, get_mean_faster(raw))
    ...
    ax.plot(lin, sens)
    ax.set_ylim(0,350)
    plt.title('Number of stations marked as outlier by average correlation')
    plt.xlabel('Minimum correlation')
    plt.ylabel('Number of stations marked as outlier')
    plt.savefig('figures/Quality measures/corrsens.png', dpi=600)
...
#   
#fig, ax = plt.subplots() 
#ax.plot([0,1], label = 'Measured runtime')
#ax.plot([0,0.5], 'r', label = 'Quadratic fit')
#ax.set_ylim(0,2)
#plt.plot()
#plt.legend()
#plt.savefig('figures/legend.png', dpi=600)
#
#mean_data = get_mean_faster(raw)
#mean_dataseries = pd.Series(mean_data[1], index=raw[0]['measurements'].index)
...
#plt.savefig('figures/averagedifferenceknmiunprocessedaapril.png', dpi=600)   
...
#gmap.scatter(lats, longs, color='darkorange',size=200, marker=False)
#
#gmap.draw("mapred.html")
#
#
